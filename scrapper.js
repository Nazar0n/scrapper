...
const mainboards = ['B350', 'B450', 'B550', 'X370', 'X470', 'X570', 'A320', 'A520'].map(
    (model) => ({
        name: model,
        keyword: model,
...
async function safeWillhabenSearch(builder, retries = 3) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            return await builder.search();
        } catch (err) {
            const isNetworkError =
                err.code === 'ECONNRESET' || err.code === 'ETIMEDOUT' || err.type === 'system';
            ...
            console.warn(
                `⚠ Willhaben network error (attempt ${attempt}/${retries}) → retrying...`,
            );
...
async function monitorTask(task) {
    ...
    try {
        const results = await safeWillhabenSearch(
...
(async () => {
    ...
    while (true) {
        ...
        if (isFirstCycle) {
            console.log('✔ Перший цикл завершено — тепер сповіщення будуть надсилатися');
            isFirstCycle = false;
        }
